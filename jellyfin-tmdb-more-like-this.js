...
`;
  ...
  function findInsertTarget() {
    ...
    if (sec && sec.parentNode && visible(sec)) return { parent: sec.parentNode, before: sec };
    const sim = best(qa('#similarCollapsible'));
    if (sim && sim.parentNode) return { parent: sim.parentNode, before: sim };
    return null;
  }
  ...
  function ensureMounted(itemId, sid, target) {
    ...
    if (!block) { block = createBlock(itemId, sid); target.parent.insertBefore(block, target.before); }
    else if (block.parentNode !== target.parent || block.nextSibling !== target.before) target.parent.insertBefore(block, target.before);
  }
  ...
  async function run() {
    ...
    if (existing && existing.dataset.serverId === sid && existing.isConnected && visible(existing)) {
      const tgt = findInsertTarget();
      if (tgt && existing.parentNode === tgt.parent && existing.nextSibling === tgt.before) return;
      if (!tgt) return;
    }
