(function () {
   ...
   */
  const TMDB_API_KEY = '2225cff2723b5b4b5ee6e5dcd8f5b1bf';
  ...
  const CFG = { styleId: 'jf-tr-style-v3', root: '[data-jf-tr-root="1"]', watchDogMs: 800, maxWaitMs: 12000, readyAnchorWaitMs: 2200, reapplyDelayMs: 250, activeMs: 20000, moThrottleMs: 150 };
  ...
  async function batchFetch(ids, type) {
    const res = await api('/Items?ids=' + ids.map(encodeURIComponent).join(',') + '&userId=' + encodeURIComponent(userId()) + '&Fields=PrimaryImageAspectRatio&EnableTotalRecordCount=false');
